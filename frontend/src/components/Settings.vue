<template>
    <div id="physics-controls" data-cy="settings-panel">
        ...
        <div class="tabs">
            ...
            <button class="tab" :class="{ active: activeTab === 'training' }" @click="activeTab = 'training'">Training
            </button>
        </div>
        ...
        <div v-show="activeTab === 'settings'" class="tab-content">
            ...
            <div v-if="groupsExpanded.flipper" class="group-content">
                ...
                <div class="slider-container">
                    ...
                    <input type="range" :value="physics.flipper_resting_angle"
                           @input="updatePhysics('flipper_resting_angle', parseFloat($event.target.value))" min="-60"
                           max="0" step="1"
                           :disabled="stats.is_training">
                </div>
                <div class="slider-container">
                    ...
                    <input type="range" :value="physics.flipper_stroke_angle"
                           @input="updatePhysics('flipper_stroke_angle', parseFloat($event.target.value))" min="10"
                           max="90" step="1"
                           :disabled="stats.is_training">
                </div>
                <div class="slider-container">
                ...
                <div
                    style="margin-bottom: 10px; margin-top: 15px; color: #aaa; font-size: 0.8em; border-top: 1px dashed #333; padding-top: 10px;">
                ...
                <div style="margin-bottom: 10px; margin-top: 10px; color: #aaa; font-size: 0.8em;">Right Flipper
                    Position
                </div>
                <div class="slider-container">
                    <div class="slider-label">
            ...
            <!-- Table Mechanics Group -->
            <div class="group-header" @click="toggleGroup('mechanics')">
                <span>Table Mechanics</span>
                <span class="arrow" :class="{ rotated: groupsExpanded.mechanics }">▼</span>
            </div>
            <div v-if="groupsExpanded.mechanics" class="group-content">
                <div class="slider-container">
                    <div class="slider-label">
                        <span>Bumper Respawn</span>
                        <span>{{ formatNumber(physics.bumper_respawn_time, 1) }}s</span>
                    </div>
                    <input type="range" min="1.0" max="30.0" step="0.5" :value="physics.bumper_respawn_time"
                           @input="updatePhysics('bumper_respawn_time', parseFloat($event.target.value))"
                           :disabled="stats.is_training">
                </div>
                <div class="slider-container">
                    <div class="slider-label">
                        <span>Drop Target Wait</span>
                        <span>{{ formatNumber(physics.drop_target_cooldown, 1) }}s</span>
                    </div>
                    <input type="range" min="0.5" max="10.0" step="0.5" :value="physics.drop_target_cooldown"
                           @input="updatePhysics('drop_target_cooldown', parseFloat($event.target.value))"
                           :disabled="stats.is_training">
                </div>
                <div
                    style="margin-bottom: 10px; margin-top: 15px; color: #aaa; font-size: 0.8em; border-top: 1px dashed #333; padding-top: 10px;">
                ...
                <div class="slider-container"
                     style="justify-content: space-between; align-items: center; border-top: 1px dashed #333; padding-top: 10px; margin-top: 10px;">
                    <div class="slider-label">
            ...
            <div v-if="groupsExpanded.camera" class="group-content">
                <div style="margin-bottom: 10px; color: #4caf50; font-size: 0.9em; font-weight: bold;">2D View Camera
                </div>
                <div class="slider-container">
                    <div class="slider-label">
                ...
                <div class="setting-group" style="margin-top: 10px; border-top: 1px solid #333; padding-top: 10px;">
                    ...
                    <div style="display: flex; gap: 5px;">
                        ...
                        <button @click="savePreset" :disabled="stats.is_training"
                                style="background: #27ae60; border: none; color: white; padding: 5px 10px; cursor: pointer;">
                            Save
                        </button>
                        <button @click="deletePreset" :disabled="stats.is_training"
                                style="background: #c0392b; border: none; color: white; padding: 5px 10px; cursor: pointer;">
                            Del
                        </button>
                    </div>
                </div>
        ...
        <div v-show="activeTab === 'training'" class="tab-content">
            ...
            <div v-if="groupsExpanded.rewards" class="group-content">
                <div class="slider-container">
                    <div class="slider-label">
                        <span>Score Log Scale</span>
                        <span>{{ formatNumber(rewards.score_log_scale, 3) }}</span>
                    </div>
                    <input type="range" min="0.001" max="0.5" step="0.001" :value="rewards.score_log_scale"
                           @input="updateRewards('score_log_scale', parseFloat($event.target.value))">
                </div>
                <div class="slider-container">
                    <div class="slider-label">
                        <span>Combo Increase Factor</span>
                        <span>{{ formatNumber(rewards.combo_increase_factor, 2) }}</span>
                    </div>
                    <input type="range" min="0.01" max="1.0" step="0.01" :value="rewards.combo_increase_factor"
                           @input="updateRewards('combo_increase_factor', parseFloat($event.target.value))">
                </div>
                <div class="slider-container">
                    <div class="slider-label">
                        <span>Multiplier Increase Factor</span>
                        <span>{{ formatNumber(rewards.multiplier_increase_factor, 2) }}</span>
                    </div>
                    <input type="range" min="0.1" max="2.0" step="0.1" :value="rewards.multiplier_increase_factor"
                           @input="updateRewards('multiplier_increase_factor', parseFloat($event.target.value))">
                </div>
                <div class="slider-container">
                    <div class="slider-label">
                        <span>Flipper Penalty</span>
                        <span>{{ formatNumber(rewards.flipper_penalty, 5) }}</span>
                    </div>
                    <input type="range" min="0.00001" max="0.001" step="0.00001" :value="rewards.flipper_penalty"
                           @input="updateRewards('flipper_penalty', parseFloat($event.target.value))">
                </div>
                <div class="slider-container">
                    <div class="slider-label">
                        <span>Bumper Hit</span>
                        <span>{{ formatNumber(rewards.bumper_hit, 2) }}</span>
                    </div>
                    <input type="range" min="0.1" max="2.0" step="0.1" :value="rewards.bumper_hit"
                           @input="updateRewards('bumper_hit', parseFloat($event.target.value))">
                </div>
                <div class="slider-container">
                    <div class="slider-label">
                        <span>Drop Target Hit</span>
                        <span>{{ formatNumber(rewards.drop_target_hit, 2) }}</span>
                    </div>
                    <input type="range" min="0.1" max="3.0" step="0.1" :value="rewards.drop_target_hit"
                           @input="updateRewards('drop_target_hit', parseFloat($event.target.value))">
                </div>
                <div class="slider-container">
                    <div class="slider-label">
                        <span>Rail Hit</span>
                        <span>{{ formatNumber(rewards.rail_hit, 2) }}</span>
                    </div>
...
const emit = defineEmits<{
  (e: 'update-physics', param: string, value: any): void
  (e: 'update-rewards', rewards: Record<string, number>): void
  (e: 'reset-config'): void
  (e: 'start-training', config: any): void
  ...
  (e: 'save-layout'): void
  (e: 'save-layout-as', name: string): void
...
watch(() => props.stats, (newStats: GameStats | undefined) => {
    if (!newStats) return
    ...
    if (!newStats.is_training && trainingChartData.mean_reward.length > 0) {
        ...
        trainingChartData.loss.push([timestamp, s.loss || 0])
        trainingChartData.value_loss.push([timestamp, s.value_loss || 0])
        trainingChartData.policy_loss.push([timestamp, s.policy_gradient_loss || 0])
        trainingChartData.entropy_loss.push([timestamp, s.entropy_loss || 0])
